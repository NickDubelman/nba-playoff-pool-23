<script>
	...
	const greenRedInterpolator = d3.interpolateHsl('rgba(255, 0, 0, 0.5)', 'rgba(0, 255, 0, 0.5)')
	...
	const ppgColor = d3
		.scaleSequential()
		.domain(d3.extent(participantScores.map((p) => (p.points ? p.points / p.gamesPlayed : 0))))
		.interpolator(greenRedInterpolator)
