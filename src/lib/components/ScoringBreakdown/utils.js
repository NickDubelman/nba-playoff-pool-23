...
export function up(svg, d, xScale, xAxis, width) {
	...
	exit.selectAll('g').transition(transition2).attr('transform', stack(d.index, xScale))
	...
	const enter = bar(svg, down, d.parent, '.exit', xScale, xAxis, width).attr('fill-opacity', 0)
...
export function down(svg, d, doAnimation, xScale, xAxis, width) {
	...
	const enter = bar(svg, down, d, '.y-axis', xScale, xAxis, width).attr('fill-opacity', 0)
